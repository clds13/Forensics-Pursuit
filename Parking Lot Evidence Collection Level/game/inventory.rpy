init -10 python:
    ...
    def use_item(usable, action) -> None:
        ...
        """
        if globals().get("knife_dialogue_style", False) and not globals().get("knife_interactions_enabled", True):
            return
...
screen inventory_slot(item=None):
    ...
    $ is_lifting_tape = (image_name == "lifting_tape")
    $ icon_zoom = 0.30 if is_lifting_tape else 0.45
    ...
    fixed:
        ...
        if image_name != "":
            if is_lifting_tape:
                add Transform(image_name, zoom=icon_zoom, xalign=0.5, yalign=0.5)
            else:
                add Transform(image_name, zoom=icon_zoom, xoffset=20, yoffset=20)
